...
export default function SupportWidget() {
    ...
    return (
        <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end">
            {isOpen && (
                <div className="mb-4 flex w-[360px] max-w-[calc(100vw-32px)] flex-col overflow-hidden rounded-[24px] border border-neutral-200 bg-white shadow-2xl dark:border-white/10 dark:bg-[#121212] sm:w-[400px]">
                    {/* Header */}
                    <div className="flex items-center justify-between border-b border-neutral-100 bg-neutral-50 px-4 py-3 dark:border-white/5 dark:bg-white/[0.02]">
                        ...
                        <button onClick={() => setIsOpen(false)} className="rounded-lg p-1.5 text-neutral-400 hover:bg-neutral-200 dark:text-white/40 dark:hover:bg-white/10">
                            <X size={16} weight="bold" />
                        </button>
                    </div>

                    {/* Body */}
                    <div className="flex h-[400px] flex-col bg-white dark:bg-transparent">
                        {isCreating ? (
                            <form onSubmit={handleCreate} className="flex h-full flex-col p-4">
            ...
            <button
                ...
                className={`flex h-[56px] w-[56px] items-center justify-center rounded-full bg-[#9EEA6C] shadow-2xl shadow-[#9EEA6C]/20 transition-all hover:scale-110 active:scale-95 ${isOpen ? 'rotate-90' : 'rotate-0'}`}
            >
                {isOpen ? (
                    <X size={24} weight="bold" className="text-[#0a0a0a]" />
                ) : (
                    <ChatCircleDots size={28} weight="duotone" className="text-[#0a0a0a]" />
                )}
            </button>
        </div>
    );
